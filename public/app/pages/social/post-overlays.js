...
export function PostOverlays() {
  ...
  const closeEditor = () => nav({ edit: null, on: null, dup: null });
  ...
  return html`
    ...
    <${Overlay} open=${!!editing} onClose=${closeEditor} title=${editTitle} phone="full" size="lg">
      ${editing && html`<${PostEditor} key=${`${editing}-${query.dup ?? ''}`} id=${editing === 'new' ? null : editing} on=${query.on ?? null}
        dup=${query.dup ?? null} platformHint=${query.platform} onTitle=${setEditTitle}
        onDone=${(/** @type {any} */ saved, /** @type {any} */ next) => {
          if (next?.dup) nav({ edit: 'new', dup: next.dup, on: null, post: null });
          else if (saved) nav({ edit: null, on: null, dup: null, post: saved.id });
          else closeEditor();
        }} />`}
    </${Overlay}>`;
}
